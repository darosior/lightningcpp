...
public:
    ...
    /** Create an invoice for `msats` millisatoshis with `label` and `description` with optional `expiry` seconds (default 1 hour), optional `fallbacks` address list (default empty list) and optional `preimage` (default autogenerated) */
